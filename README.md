...
* Jumping Knowledge GCN (JK-Net) is supported. The JK-Net in the [original paper](https://arxiv.org/abs/1806.03536) adopts the neighbor sampling strategy of GraphSAGE, where neighbor explosion in deeper layers is **not** resolved. Here in this codebase, we demonstrate that graph sampling based minibatch of GraphSAINT can be applied to JK-Net architecture to improve training scalability w.r.t. GCN depth. Check out `./train_config/explore/reddit4_jk_e.yml` for a 4-layer JK-Net achieving **0.969** F1-Micro on Reddit. The total training time (using independent edge sampler) is under 45 seconds!
