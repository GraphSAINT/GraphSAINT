...
def train(train_phases, model, minibatch):
    ...
    for ip, phase in enumerate(train_phases):
        ...
        for e in range(epoch_ph_start, int(phase['end'])):
            ...
            loss_val, f1mic_val, f1mac_val = evaluate_full_batch(model, minibatch, mode='val')
    ...
    loss_test, f1mic_test, f1mac_test = evaluate_full_batch(model, minibatch, mode='test')
...
if __name__ == '__main__':
    ...
    model, minibatch = prepare(train_data, train_params, arch_gcn)
    train(train_phases, model, minibatch)
